...
import float4096 as f4096
from float4096 import Float4096 as F4096, Float4096Array, GRAElement

# Set up logging
logging.basicConfig(filename='symbolic_cosmo_fit.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s', force=True)

def D(n, beta, r=F4096(1), k=F4096(1), Omega=F4096(1), base=F4096(2), scale=F4096(1)):
    try:
        r_n = GRAElement(n + beta, Omega=Omega, base=base)
        val = scale * r_n._value
        if not val.is_finite() or val <= F4096(0):
            return F4096("1e-30")
        return val * (r ** k)
    except Exception:
        return F4096("1e-30")
...
def generate_emergent_constants(n_max=5000, beta_steps=10, r_values=[0.5, 1.0, 2.0], k_values=[0.5, 1.0, 2.0], Omega=1.0, base=2, scale=1.0):
    candidates = []
    n_values = f4096.linspace(F4096(0), F4096(n_max), 500)
    beta_values = f4096.linspace(F4096(0), F4096(1), beta_steps)
            ...
            for r in r_values:
                for k in k_values:
                    ...
                    if val is not None and val.is_finite():
                        candidates.append({
                            'n': float(n), 'beta': float(beta), 'value': float(val), 'r': float(r), 'k': float(k), 'scale': float(scale)
                        })
                    val_inv = f4096.inv(D(n, beta, r, k, Omega, base, scale))
                    if val_inv is not None and val_inv.is_finite():
                        candidates.append({
                            'n': float(n), 'beta': float(beta), 'value': float(val_inv), 'r': float(r), 'k': float(k), 'scale': float(scale)
                        })
...
def match_to_codata(df_emergent, df_codata, tolerance=0.05, batch_size=100):
    ...
    for start in range(0, len(df_codata), batch_size):
        ...
        for _, codata_row in tqdm(batch.iterrows(), total=len(batch), desc=f"Matching constants batch {start//batch_size + 1}"):
            value = F4096(codata_row['value'])
            mask = (f4096.abs(Float4096Array(df_emergent['value']) - value) / f4096.max(f4096.abs(value), F4096("1e-30")) < F4096(tolerance))
            matched = df_emergent[np.array(mask)]
            for _, emergent_row in matched.iterrows():
                error = f4096.abs(F4096(emergent_row['value']) - value)
                rel_error = error / f4096.max(f4096.abs(value), F4096("1e-30"))
                matches.append({
                    ...
                    'bad_data': float(rel_error) > 0.5 or (codata_row['uncertainty'] is not None and float(f4096.abs(F4096(codata_row['uncertainty']) - error)) > 10 * codata_row['uncertainty']),
                    'bad_data_reason': f"High rel_error ({float(rel_error):.2e})" if float(rel_error) > 0.5 else f"Uncertainty deviation ({codata_row['uncertainty']:.2e} vs. {float(error):.2e})" if (codata_row['uncertainty'] is not None and float(f4096.abs(F4096(codata_row['uncertainty']) - error)) > 10 * codata_row['uncertainty']) else ""
                })
        try:
...
def check_physical_consistency(df_results):
    bad_data = []
    relations = [
        ('Planck constant', 'reduced Planck constant', lambda x, y: float(f4096.abs(F4096(x['scale']) / F4096(y['scale']) - F4096(2) * f4096.pi())), 0.1, 'scale ratio vs. 2π'),
        ('proton mass', 'proton-electron mass ratio', lambda x, y: float(f4096.abs(F4096(x['n']) - F4096(y['n']) - f4096.log10(F4096(1836)))), 0.5, 'n difference vs. log(proton-electron ratio)'),
        ('Fermi coupling constant', 'weak mixing angle', lambda x, y: float(f4096.abs(F4096(x['scale']) - F4096(y['scale']) / f4096.sqrt(F4096(2)))), 0.1, 'scale vs. sin²θ_W/√2'),
        ('tau energy equivalent', 'tau mass energy equivalent in MeV', lambda x, y: float(f4096.abs(F4096(x['codata_value']) - F4096(y['codata_value']))), 0.01, 'value consistency'),
        ('proton mass', 'electron mass', 'proton-electron mass ratio', 
         lambda x, y, z: float(f4096.abs(F4096(z['n']) - f4096.abs(F4096(x['n']) - F4096(y['n'])))), 10.0, 'n inconsistency for mass ratio'),
        ('fine-structure constant', 'elementary charge', 'Planck constant', 
         lambda x, y, z: float(f4096.abs(F4096(x['codata_value']) - (F4096(y['codata_value']) ** F4096(2)) / (F4096(4) * f4096.pi() * F4096(8.854187817e-12) * F4096(z['codata_value']) * F4096(299792458)))), 0.01, 'fine-structure vs. e²/(4πε₀hc)'),
        ('Bohr magneton', 'elementary charge', 'Planck constant', 
         lambda x, y, z: float(f4096.abs(F4096(x['codata_value']) - F4096(y['codata_value']) * F4096(z['codata_value']) / (F4096(2) * F4096(9.1093837e-31)))), 0.01, 'Bohr magneton vs. eh/(2m_e)'),
        ('speed of light in vacuum', None, lambda x: float(f4096.abs(F4096(x['codata_value']) - F4096(299792458))), 0.01, 'speed of light deviation'),
        ('Newtonian constant of gravitation', None, lambda x: float(f4096.abs(F4096(x['codata_value']) - F4096(6.6743e-11))), 1e-12, 'G deviation')
    ]
...
def total_error(params, df_subset):
    ...
    return float(f4096.mean(valid_errors)) if len(valid_errors) > 0 else float('inf')

def process_constant(row, r, k, Omega, base, scale):
    try:
        name, value, uncertainty, unit = row['name'], F4096(row['value']), F4096(row['uncertainty']) if pd.notnull(row['uncertainty']) else F4096(0.0), row['unit']
        abs_value = f4096.abs(value)
        sign = F4096(1) if value >= F4096(0) else F4096(-1)
        result = f4096.invert_D(abs_value, r=r, k=k, Omega=Omega, base=base, scale=scale)
        if result[0] is None:
            logging.warning(f"No valid fit for {name}")
            return {
                'name': name, 'codata_value': float(value), 'unit': unit, 'n': None, 'beta': None, 'emergent_value': None,
                'error': None, 'rel_error': None, 'codata_uncertainty': float(uncertainty), 'emergent_uncertainty': None,
        ...
        approx = D(n, beta, r_local, k_local, Omega, base, scale * dynamic_scale) if value > F4096(0) else f4096.inv(D(n, beta, r_local, k_local, Omega, base, scale * dynamic_scale))
        if approx is None:
        ...
        approx *= sign
        error = f4096.abs(approx - value)
        rel_error = error / f4096.max(f4096.abs(value), F4096("1e-30")) if abs(value) > F4096(0) else F4096('inf')
        bad_data = False
        bad_data_reason = ""
        if rel_error > F4096(0.5):
            bad_data = True
        ...
        if emergent_uncertainty is not None and uncertainty is not None:
            if emergent_uncertainty > uncertainty * F4096(20) or emergent_uncertainty < uncertainty / F4096(20):
                bad_data = True
                bad_data_reason += f"Uncertainty deviates from emergent ({float(emergent_uncertainty):.2e} vs. {float(uncertainty):.2e}); "
        return {
            'name': name, 'codata_value': float(value), 'unit': unit, 'n': float(n), 'beta': float(beta), 'emergent_value': float(approx),
            'error': float(error), 'rel_error': float(rel_error), 'codata_uncertainty': float(uncertainty), 
            'emergent_uncertainty': float(emergent_uncertainty), 'scale': float(scale * dynamic_scale),
            'bad_data': bad_data, 'bad_data_reason': bad_data_reason, 'r': float(r_local), 'k': float(k_local)
        }
    except Exception as e:
...
def symbolic_fit_all_constants(df, base=2, Omega=1.0, r=1.0, k=1.0, scale=1.0, batch_size=100):
    ...
    base, Omega, r, k, scale = [F4096(x) for x in (base, Omega, r, k, scale)]
    results = []
    ...
    if not df_results.empty:
        ...
        for name in df_results['name'].unique():
            ...
            if df_results.loc[mask, 'codata_uncertainty'].notnull().any():
                uncertainties = Float4096Array(df_results.loc[mask, 'codata_uncertainty'].dropna())
                if len(uncertainties) > 0:
                    Q1, Q3 = f4096.percentile(uncertainties, [F4096(25), F4096(75)])
                    IQR = Q3 - Q1
                    outlier_mask = (uncertainties < Q1 - F4096(1.5) * IQR) | (uncertainties > Q3 + F4096(1.5) * IQR)
                    if any(outlier_mask):
...
def s(z, s0, beta):
    return s0 * (F4096(1) + z) ** (-beta)

def G(z, k, r0, Omega0, s0, alpha, beta):
    return Omega(z, Omega0, alpha) * (k ** F4096(2)) * r0 / s(z, s0, beta)

def H(z, k, r0, Omega0, s0, alpha, beta):
    Om_m = F4096("0.3")
    Om_de = F4096("0.7")
    Gz = G(z, k, r0, Omega0, s0, alpha, beta)
    Hz_sq = (F4096(H0) ** F4096(2)) * (Om_m * Gz * (F4096(1) + z) ** F4096(3) + Om_de)
    return f4096.sqrt(Hz_sq)

def emergent_c(z, Omega0, alpha, gamma):
    return F4096(c0_emergent) * (Omega(z, Omega0, alpha) / Omega0) ** gamma * lambda_scale

def compute_luminosity_distance_grid(z_max, params, n=500):
    k, r0, Omega0, s0, alpha, beta, gamma = params
    z_grid = f4096.linspace(F4096(0), z_max, n)
    c_z = emergent_c(z_grid, Omega0, alpha, gamma)
    H_z = H(z_grid, k, r0, Omega0, s0, alpha, beta)
    integrand_values = c_z / H_z
    integral_grid = Float4096Array([F4096(0)] + [sum((integrand_values[:-1] + integrand_values[1:]) / F4096(2) * np.diff([float(z) for z in z_grid]))])
    d_c = interp1d([float(z) for z in z_grid], [float(i) for i in integral_grid], kind='cubic', fill_value="extrapolate")
    return lambda z: F4096(1 + z) * F4096(d_c(float(z)))

def model_mu(z_arr, params):
    d_L_func = compute_luminosity_distance_grid(f4096.max(z_arr), params)
    d_L_vals = Float4096Array([d_L_func(float(z)) for z in z_arr])
    return F4096(5) * f4096.log10(d_L_vals) + F4096(25)
...
def main():
    ...
    lc_data = pd.read_csv(supernova_file, delim_whitespace=True, comment='#', dtype={'zcmb': float, 'mb': float, 'dmb': float})
    ...
    fitted_params = {
        ...
        'c0': float(f4096.phi ** (F4096(2.5) * F4096(6))), 'M': -19.3
    }
    params_reconstructed = {}
    for name, val in fitted_params.items():
        if name == 'M':
            params_reconstructed[name] = F4096(val)
            continue
        n, beta, _, _, r, k = f4096.invert_D(F4096(val))
        params_reconstructed[name] = D(n, beta, r, k) if name != 'c0' else f4096.phi ** (F4096(2.5) * n)

    global H0, c0_emergent, lambda_scale, lambda_G
    H0 = params_reconstructed['H0']
    c0_emergent = params_reconstructed['c0']
    lambda_scale = F4096(299792.458) / c0_emergent
    lambda_G = F4096(6.6743e-11) / G(F4096(0), params_reconstructed['k'], params_reconstructed['r0'], 
    ...
    z_grid = f4096.linspace(F4096(0), f4096.max(z), 300)
