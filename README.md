...
Install Dependencies:Save the following in requirements.txt (or use the provided file):
numpy>=1.21.0
sympy>=1.8
scipy>=1.7.0
pandas>=1.3.0
...
This command removes the float4096 package and optionally removes the cloned repository directory. It assumes the user is in the Float4096 directory after running the Git clone command.
Linux/macOS (Bash/Zsh):
cd Float4096 && [ -d venv ] && source venv/bin/activate && pip uninstall -y float4096 && deactivate && cd .. && rm -rf Float4096

Windows (Git Bash/WSL):
cd Float4096 && [ -d venv ] && source venv/Scripts/activate && pip uninstall -y float4096 && deactivate && cd .. && rm -rf Float4096
...
[ -d venv ] && source venv/bin/activate (or if (Test-Path venv) { . .\venv\Scripts\Activate.ps1 }): Activates the virtual environment if it exists.
...
If you encounter issues with the virtual environment, you can recreate it manually (replace Owner with your username):
Delete Existing Environment
Linux/macOS:
rm -rf /home/Owner/Float4096/venv

Windows (PowerShell):
Remove-Item -Recurse -Force C:\Users\Owner\Float4096\venv

Create New Virtual Environment
python3 -m venv /home/Owner/Float4096/venv  # Linux/macOS
# or
python -m venv C:\Users\Owner\Float4096\venv  # Windows

Activate and Install Dependencies
Linux/macOS:
source /home/Owner/Float4096/venv/bin/activate
pip install --upgrade pip
pip install -r requirements.txt

Windows (Git Bash/WSL):
source /c/Users/Owner/Float4096/venv/Scripts/activate
pip install --upgrade pip
pip install -r requirements.txt
...
License
This project is licensed under the zchg.org License. See zchg.org License for details. [Note: Please provide a summary or full text of the license terms, as the link may not be accessible to all users.]
